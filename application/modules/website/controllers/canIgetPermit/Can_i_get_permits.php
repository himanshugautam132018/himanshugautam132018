...
class Can_i_get_permits extends MY_Controller
{
    ...
    /*
| -------------------------------------------------------------------------
|Canb I Get Section start

Note Point : 
1 : ye Calculation Can I Get Permit Section ki hai . ye calculation 2 point check karti hai first no permit and second Annual permit. 
2  : customer jab truck details bhar k submit karta hai tab calculation yahan se start hota hai se.
3 : sabse pahle calculation ki sabhi row get kar lete hai Database se.
4 : uske bad Ek Foreach Loop chalate hai ,sirf unhi state ko check karte hai jinme permit type "No" hota h , No permit Type ki ID=15 hai jo default set hai. 
5 : Fir iske bad Annual permit get karte hai usi state ki , ye sab Calculation Foreach k andar ho rahi h.
6 : ye Calculation sabhi truck type k lia common hai.
7 : iske bad hum check karte hai user ki input  value agar "NO" permit ki value se less than hai to next condition ko check karega ,agar value user input value No permit ki value se
Greater than hai to usko vahin stop kar dega ,or annual permit main check karne chala jayega , ab vo check karega  user input value  annual permit ki value se greter than hai under 
violation main message show karega agar less than hai Annual permit Yes ho jayega 
    ...
    public function can_i_get_permit_calculation()
    {
        ...
        foreach ($data['permit_calculation'] as $calculation) {
            ...
            if ($calculation['xcp_permit_type_id'] == 15) {
                ...
                $annual_permit = read_data_where_row("xcmg_permit_calculation", array("xcp_state_id" => $calculation['xcp_state_id'], "xcp_permit_type_id" => 17));
                $annual_permit->xcp_steer_axle_lb_in = !empty($annual_permit->xcp_steer_axle_lb_in) ? $annual_permit->xcp_steer_axle_lb_in : $annual_permit->xcp_single_axle_lb_in;
                $annual_permit->xcp_steer_axle_lb = !empty($annual_permit->xcp_steer_axle_lb) ? $annual_permit->xcp_steer_axle_lb : $annual_permit->xcp_single_axle_lb;
                ...
                if (
                    ...
                    if (empty($annual_permit->xcp_height_limit_ft)  || num_condition_operator($postdata['overallHeight'], $calculation['xcp_height_rule_operator'], $calculation['xcp_height_limit_ft'])) {
                        if (empty($calculation['xcp_length_limit_ft'])) {
                            if (empty($calculation['xcp_front_overhang_ft'])) {
                                if (empty($calculation['xcp_rear_overhang_ft'])) {
                                    if (empty($calculation['xcp_gvw'])) {
                                        if (empty($calculation['xcp_steer_axle_lb'])) {
                                            ...
                                            //cheked
                                            if (empty($annual_permit->xcp_steer_axle_lb)  || num_condition_operator($xcp_steer_axle_lb, $calculation['xcmg_overall_operator'], $calculation['xcp_steer_axle_lb'])) {
                                                $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                                $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                                $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                                $response[$calculation['xcp_state_id']]['violation'] = "";
                                        ...
                                        if (empty($annual_permit->xcp_gvw)  || num_condition_operator($postdata['gvw'], $calculation['xcmg_overall_operator'], $calculation['xcp_gvw'])) {
                                    ...
                                    if (empty($annual_permit->xcp_rear_overhang_ft)  || num_condition_operator($postdata['rearoverhang'], $calculation['xcmg_overall_operator'], $calculation['xcp_rear_overhang_ft'])) {
                                        $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                        $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                        $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                        $response[$calculation['xcp_state_id']]['violation'] = "";
                                ...
                                if (empty($annual_permit->xcp_front_overhang_ft)  || num_condition_operator($postdata['frontoverhang'], $calculation['xcmg_overall_operator'], $calculation['xcp_front_overhang_ft'])) {
                                    $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                    $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                    $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                    $response[$calculation['xcp_state_id']]['violation'] = "";
                            ...
                            if (empty($annual_permit->xcp_length_limit_ft)  || num_condition_operator($postdata['overall_crane_length'], $calculation['xcmg_overall_operator'], $calculation['xcp_length_limit_ft'])) {
                                if (empty($calculation['xcp_front_overhang_ft'])) {
                                    if (empty($calculation['xcp_rear_overhang_ft'])) {
                                        if (empty($calculation['xcp_gvw'])) {
                                            if (empty($calculation['xcp_steer_axle_lb'])) {
                                                        ...
                                                        if (empty($calculation['xcp_steer_axle_lb_in'])) {  //call when  empty xcp_steer_axle_lb_in
                                                            if (empty($tab_calculation_for_weight)) {
                                                                if (empty($tab_calculation_lb_in)) {
                                                                    ...
                                                                    if (empty($annual_permit->{$tab_db_name_lb_in})  || num_condition_operator($all_axle_tire_size, $calculation['xcmg_overall_operator'], $tab_calculation_lb_in)) {
                                                                        $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                                                        $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                                                        $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                                                        $response[$calculation['xcp_state_id']]['violation'] = "";
                                                                ...
                                                                if (empty($annual_permit->{$tab_db_name_lb_in})  || num_condition_operator($axle_weight_sum, $calculation['xcmg_overall_operator'], $tab_calculation_for_weight)) {
                                                                    if (empty($tab_calculation_lb_in)) {
                                                                        $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                                                        $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                                                        $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                                                        ...
                                                                        if (empty($annual_permit->{$tab_calculation_lb_in})  || num_condition_operator($all_axle_tire_size, $calculation['xcmg_overall_operator'], $tab_calculation_lb_in)) {
                                                                            $response[$calculation['xcp_state_id']]['state_name'] = $state_list->state_name;
                                                                            $response[$calculation['xcp_state_id']]['permit_required'] = "NO";
                                                                            $response[$calculation['xcp_state_id']]['annual_permit'] = "N/A";
                                                                            $response[$calculation['xcp_state_id']]['violation'] = "";
                                                            ...
                                                            if (empty($annual_permit->xcp_steer_axle_lb_in)  || num_condition_operator($xcp_steer_axle_tire_width, $calculation['xcmg_overall_operator'], $calculation['xcp_steer_axle_lb_in'])) {
                                                                ...
                                                                if (empty($tab_calculation_for_weight)) {
                                                                    ...
                                                                    if (empty($annual_permit->{$tab_db_name})  || num_condition_operator($axle_weight_sum, $calculation['xcmg_overall_operator'], $tab_calculation_for_weight)) {
                                                                        if (empty($tab_calculation_lb_in)) {
                                    ...
                                    if (empty($annual_permit->xcp_front_overhang_ft)  || num_condition_operator($postdata['frontoverhang'], $calculation['xcmg_overall_operator'], $calculation['xcp_front_overhang_ft'])) {
                                        if (empty($annual_permit->xcp_front_overhang_ft)  || num_condition_operator($postdata['rearoverhang'], $calculation['xcmg_overall_operator'], $calculation['xcp_rear_overhang_ft'])) {
                                            if (empty($calculation['xcp_gvw']) && empty($postdata['gvw'])) {
                                                ...
                                                if (empty($annual_permit->xcp_gvw)  || num_condition_operator($postdata['gvw'], $calculation['xcp_gvw_operator'], $calculation['xcp_gvw'])) {
