...
class Manage_state extends MY_Controller
{
    ...
    public function create_update_user_information_section()
    {
        ...
        // $this->form_validation->set_rules('permit_type[]', 'Permit Type ', 'required');
        // foreach ($postdata['permit_type'] as $id => $data) {
        //     $this->form_validation->set_rules('permit_type[' . $id . ']', 'Permit Type', 'required|trim');
        // }
        if ($this->form_validation->run() == FALSE) {
            ...
            foreach ($postdata['permit_type'] as $id => $data) {
                if (empty($data)) {
                    $error = 'All Permit Type field mandatory';
                }
            }
            if (!empty($error)) {
                echo json_encode(array('status' => false, 'message' => '<div class="alert alert-danger">All field of permit type are Mandatory</div>'));
                die;
            }
            $insert_data = array(
            ...
            if (!empty($postdata['state_id'])) {
                ...
                if ($last_inserted_id) {
                    foreach ($postdata['permit_type'] as $permit_type) {
                        if (!empty($permit_type)) {
                            $state_permit_type[] = array(
                                "xcmg_swp_permit_type" => $permit_type,
                                "xcmg_swp_state_id" => $last_inserted_id,
                                "xcmg_swp_created" => date("Y-m-d h:i:s"),
                                "xcmg_swp_status" => 1,
                            );
                        }
                    }
                    $this->db->insert_batch('xcmg_state_wise_permit', $state_permit_type);
                }
