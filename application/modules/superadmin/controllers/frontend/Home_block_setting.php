    ...
    public function home_page_setting_information_update()
    {
        ...
        if (!empty($data['aboutus_title']) && !empty($data['aboutus_description'])) {
            ...
            $postdata = array(
                ...
                'aboutus_description' => $data['aboutus_description'],
                'aboutus_description' => $data['aboutus_description'],
                'gmc_title' => $data['gmc_title'],
                'gmc_description' => $data['gmc_description'],
                'cfd_mobile_title' => $data['cfd_mobile_title'],
                'cfd_mobile_description' => $data['cfd_mobile_description'],
                'cryptocurrencies_title' => $data['cryptocurrencies_title'],
                'cryptocurrencies_description' => $data['cryptocurrencies_description'],
                'created' => date("Y-m-d h:i:s")
            );

            if (!empty($_FILES['about_us_thumbnail']['name'])) {
                $profile_pic = upload_image_doc_helper($_FILES['about_us_thumbnail'], $this->uploadPath);
                if ($profile_pic['status'] == true) {
                    $fileData_res = $profile_pic['data'];
                    $postdata['about_us_thumbnail'] = $fileData_res['file_name'];
                } else {
                    $statusMsg = "Sorry, there was an error uploading About thumbnail image." . $profile_pic['error'];
                    echo json_encode(array('status' => false, 'msg' => '<div class="alert alert-danger">' . $statusMsg . '</div>'));
                    die;
                    
                }
            }
            if (!empty($_FILES['cfd_mobile_thumbnail']['name'])) {
                $profile_pic = upload_image_doc_helper($_FILES['cfd_mobile_thumbnail'], $this->uploadPath);
                if ($profile_pic['status'] == true) {
                    $fileData_res = $profile_pic['data'];
                    $postdata['cfd_mobile_thumbnail'] = $fileData_res['file_name'];
                } else {
                    $statusMsg = "Sorry, there was an error uploading Highly Rated CFD Mobile thumbnail image." . $profile_pic['error'];
                    echo json_encode(array('status' => false, 'msg' => '<div class="alert alert-danger">' . $statusMsg . '</div>'));
                    die;
                    
                }
            }
            if (!empty($_FILES['cryptocurrencies_thumbnail']['name'])) {
                $profile_pic = upload_image_doc_helper($_FILES['cryptocurrencies_thumbnail'], $this->uploadPath);
                if ($profile_pic['status'] == true) {
                    $fileData_res = $profile_pic['data'];
                    $postdata['cryptocurrencies_thumbnail'] = $fileData_res['file_name'];
                } else {
                    $statusMsg = "Sorry, there was an error uploading Cryptocurrencies  thumbnail image." . $profile_pic['error'];
                    echo json_encode(array('status' => false, 'msg' => '<div class="alert alert-danger">' . $statusMsg . '</div>'));
                    die;
